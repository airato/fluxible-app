...
function FluxContext(options) {
    ...
    this._currentActions = 0;
    this._allActionsDoneCallbacks = [];
}
 ...
 */
FluxContext.prototype.executeAction = function executeAction(action, payload, done, allActionsDone) {
    var self = this;
    payload = payload || {};
    this._currentActions++;
    if ( typeof allActionsDone === 'function' ) {
      this._allActionsDoneCallbacks.push(allActionsDone);
    }
    setImmediate(function executeActionImmediate() {
        debug('Executing action ' + (action.name) + ' with payload', payload);
        action(self.getActionContext(), payload, function(){
          self._currentActions--;
          done.apply(null, Array.prototype.slice.call(arguments, 0));
          if ( self._currentActions === 0 ) {
            var callbacks = self._allActionsDoneCallbacks.slice();
            self._allActionsDoneCallbacks.length = 0;
